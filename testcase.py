print("this is a test case")

print("this is the edition fo the testcase")


print("this is the third edition of the testcase")

print("this is the fourth edition of the testcase")

print("this is the fifth edition of the testcase")

print("this is the sixth edition of the testcase")

print("this is the seventh edition of the testcase")

print("this is the eighth edition of the testcase")

print("this is the ninth edition of the testcase")

print("this is the tenth edition of the testcase")

